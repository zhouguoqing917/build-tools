...
// https://github.com/primer/css/tree/main/script
...
prog
	...
	.option("--entry, -i", "Entry module")
	.example("build --entry src/index.tsx")
	.option("--target", "Specify your target environment", "browser")
...
async function normalizeOpts(opts: WatchOpts): Promise<NormalizedOpts> {
	const inputFiles = await getInputs(opts.entry, appPackageJson.source);
	return {
		...opts,
